...
I didn't quite succeed with the challenge...

Below is code that creates a `textGrob` from the longest string and takes the `grobWidth` to get the width of relevant viewports. Then I create 2 ggplots, 1 showing the actual lines and 1 using `grid_group` in a plot without points to obtain a plot only of the text. The reason why I thought of using `grid_group` in an empty plot was to obtain the y-coordinates of the first values in the line plots.

The approach almost works, but I do have some problems with the text being cut to the left. I have checked that my viewport goes all the way to the left edge (see grey grid.rect in the picture), and I cannot figure out why the text is being cut.
...
dummy_grob <- grid::textGrob(longest_str, gp = grid::gpar(fontsize = fontsize))
...
p_text <- ggplot(youthMonthType,
               aes(x = Month, y = Freq,
                   color = Var2noBreak, group = Var2noBreak)) +
...
create_plot <- function() {
  ...
  grid::pushViewport(
    grid::viewport(
      width = grid::unit(1, "npc") - text_width - grid::unit(2*2, "mm"),
      x = grid::unit(1, "npc") - grid::unit(2, "mm"),
      just = c(1, .5)
    )
  )
  ...
  grid::grid.rect(gp = grid::gpar(col = "grey70"))
  print(p_text, newpage = FALSE)
  grid::upViewport()
  return(invisible())
}
